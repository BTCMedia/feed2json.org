...
app.get('/convert', (req, res) => {
  ...
  // Check to see if cached files need to delete the files & recreated
  if (fs.existsSync(feedFilename)) {
    const file_time = fs.statSync(feedFilename).mtime.getTime();
    const now_time = new Date().getTime();

    if (now_time - file_time > (1000 * 60 * 5)) {
      fs.unlinkSync(feedFilename);
      fs.unlinkSync(jsonFilename);
      fs.unlinkSync(jsonMinFilename);
    }
  }
    ...
    let fetch = request(url, { timeout : 10000, pool : false }, (err, response, body) => {
      ...
      fs.writeFile(feedFilename, body, (err) => {
        ...
        feed2json.fromStream(stream, url, (err, data) => {
          ...
          // remove the content from the payload to speed up the requests; just not needed
          data["items"].forEach(function (article_data, index) {
            delete article_data["content_html"];
          });
